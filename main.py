...
dict_classes, number_of_students = preprocessing.read_in_csv()
...
if "" in sys.argv or len(sys.argv) != 5:
    ...
    in_person_final = alg1.find_in_person(dict_classes, number_of_students, 0.1, 3, 7, 3)
else:
    in_person_final = alg1.find_in_person(dict_classes, number_of_students, float(sys.argv[1]), int(sys.argv[2]), int(sys.argv[3]), int(sys.argv[4]))
